...
async function doMantisRelease({ options, commits, logger, lastRelease, nextRelease, env })
{
    if (options.mantisbtRelease === "Y" && (!options.taskMode || options.taskMantisbtRelease))
    {
        ...
        //
        runScripts({ options, logger }, "preMantisRelease", options.mantisbtReleasePreCommand);
        ...
        //
        const mantisChangelog = await createReleaseChangelog({ options, logger, lastRelease }, false, false);
        ...
        // Run pre msntis-release scripts if specified
        //
        runScripts({ options, logger }, "postMantisRelease", options.mantisbtReleasePostCommand);
    }
}
