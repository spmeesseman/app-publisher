...
async function doGithubRelease({ options, logger, lastRelease, env })
{
    ...
    if (options.repo === "git" && options.githubRelease === "Y" && !options.taskMode)
    {
        ...
        //
        runScripts({ options, logger }, "preGithubRelease", options.githubReleasePreCommand);
        ...
        // Run pre msntis-release scripts if specified
        //
        runScripts({ options, logger }, "postGithubRelease", options.githubReleasePostCommand);
    }
}
